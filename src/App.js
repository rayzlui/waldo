...
class App extends Component {
  constructor(props) {
    ...
    this.tagsOfSelectedPhotoFromIndex = this.tagsOfSelectedPhotoFromIndex.bind(
      this,
    );
  }
  ...
  async tagsOfSelectedPhotoFromIndex(id) {
    ...
    let selectedPhoto = data.photo;
    let tags = data.tags;
    let newtags = {};
    for (let i = 0; i < tags.length; i++) {
      let div = tags[i][1] / 1; //make it a number.
      let tagId = tags[i][0];
      let tagname = await getTagName(tagId);
  ...
  render() {
    ...
    if (this.state.photo !== null) {
      ...
      grid = (
        <GridDOM
          state={this.state}
          checkTagForGame={this.checkTagForGame.bind(this)}
          processClick={this.processClick.bind(this)}
          submitTagForPhoto={this.submitTagForPhoto.bind(this)}
          tagInput={this.tagInput.bind(this)}
        />
      );
    }

    if (this.state.game) {
      score = gameModeHeader({
        score: this.state.score,
        tagPhotoStatus: this.tagPhotoStatus.bind(this),
      });
    } else {
      game = 'TAG';
      storeTags = (
      ...
      score = (
        <h4 id="play-game-button" onClick={this.playGame.bind(this)}>
          Play Guessing Game{' '}
        </h4>
      );
    }
    ...
    return (
      <div className="application">
        ...
        <div className="tag-vs-game-button">{button}</div>
        <div className="store-tags-database">{storeTags}</div>

        <div
          className="main-container"
          style={{ position: 'relative', paddingTop: 80 }}
        >
          <div className="grid-location">{grid}</div>

          <div className="photo-location">
            {typeOfPhotoDisplay({
              photo: this.state.photo,
              data: this.state.data,
              func: this.tagsOfSelectedPhotoFromIndex,
            })}
          </div>
        </div>
      </div>
