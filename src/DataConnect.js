async function getData() {
  let data;
  try {
    let parsedata = await fetch('http://localhost:3001', { mode: 'cors' });
    data = await parsedata.json();
  } catch (err) {
    alert(
      'Unable to access server, please play with Minions for the time being.',
    );
...
async function getTagName(id) {
  try {
    let data = await fetch(`http://localhost:3001/tags/${id}`, {
      mode: 'cors',
    });
    let tag = await data.json();
    return tag[0].tag;
  } catch {
