import {needInputBox, needOptionsBox, buildSquare, buildGrid} from './GridDOM.js'
...
describe("needInputBox", ()=>{
    it('should return an null if gridnum !== state.clicked',()=>{
        const wrapper = needInputBox({gridnum: 8, state: {clicked: 10}})
        expect(wrapper).toEqual(null)
    });

    it('should return an input box if gridnum === state.clicked',()=>{
        const wrapper = shallow(needInputBox({gridnum: 10, state: {clicked: 10}}))
    ...
    it("should pass option functions into input when gridnum === state.clicked",()=>{
        var foo = jest.fn()
        var bar = jest.fn()
        const wrapper = shallow(needInputBox({gridnum: 11, state: {clicked: 11, newtag: "epsum"}, tagInput: foo, submitTagForPhoto: bar}))
...
describe('needOptionsBox', ()=>{
    it('should return null when gridnum !== state.clicked', ()=>{
        const wrapper = needOptionsBox({gridnum: 10, state: {clicked: 15}})
        expect(wrapper).toEqual(null)
    })
    it('should return an Options Component when gridnum === state.clicked', ()=>{
        const wrapper = shallow(needOptionsBox({gridnum: 10, state: {clicked: 10, tags:[{1:1},{2:2},{3:3}]}}))
...
describe('buildSqaure', () => {
    ...
    it('should return Grid component with popup2 if state.game === null, if popupbox !== null click should not work.', ()=>{
        ...
        var mock1 = jest.fn()
        const wrapper = shallow(buildSquare({needInputBox: popup2, processClick: mock1, needOptionsBox: popup1, gridnum: 4, state: {game: null, tags: [1,2,3]}}) )
    ...
    it('should return Grid component with null popup and null click if state.game !== null && state.tags[gridnum] === undefined', ()=>{
        ...
        const wrapper = shallow(buildSquare({needInputBox: popup2, needOptionsBox: popup1, processClick: mock1, gridnum: 4, state: {game: 4, tags: [0,1,2]}}) )
        expect(wrapper.props().children.props.children).toEqual(null)
    ...
    it('should return Grid component with null if state.game !== null && state.tags[gridnum] !== undefined, popup1 !== null', ()=>{
        ...
        var popup2 = jest.fn(x=>3)
        var mock1 = jest.fn()
        const wrapper = shallow(buildSquare({needInputBox: popup2, needOptionsBox: popup1, processClick: mock1, gridnum: 4, state: {game: 4, tags: [0,1,2,3,4]}}) )
...
describe("buildGrid", ()=>{
    it('should return 64 grids', ()=>{
        const grid = buildGrid({func: ()=>{"hello"}})
